...
- [x] Add `@tanstack/react-query` (and `@tanstack/react-query-devtools` in dev) to each app's `package.json`. Bun-install at root. (Deviation: monorepo already had `@tanstack/react-query` at root; added `@tanstack/react-query-devtools` and `@tanstack/query-core` at root to satisfy peer deps. Per-app `package.json` doesn't list them — the existing convention in this monorepo is root-hoisted React/Tanstack deps, mirrored across all 5 apps. Adding per-app entries would diverge from that pattern.)
- [x] Per app, create `src/lib/query-client.ts` exporting a singleton `queryClient: QueryClient` with sensible defaults (`staleTime: 30s`, `retry: 1`, `refetchOnWindowFocus: false` in dev). (Same defaults across all 5 apps; identical files.)
- [x] Per app, create `src/providers/QueryProvider.tsx` wrapping `QueryClientProvider` + `ReactQueryDevtools`. Devtools mount-gate uses `import.meta.env.DEV` (Vite's built-in DEV flag) — simpler than introducing a new `VITE_APP_ENV` just for this.
- [x] Per app, mount `<QueryProvider>` and `<AuthProvider>` in the root shell (`src/routes/__root.tsx`'s `RootShell`). Order: `<QueryProvider><AuthProvider>{children}</AuthProvider></QueryProvider>`.
- [x] Add a 401 interceptor to the api-client. In `packages/api-client/src/client.ts`, expose an `onUnauthorized?: () => void` option on `createClient` (and forward through `createVeriflyClient`); call it from `request` when `res.status === 401`. The interceptor skips paths under `/auth/*` (so a wrong-password 401 on login doesn't trigger a redirect loop) and supports a per-call `skipUnauthorizedHandler` opt-out. Also adds `setOnUnauthorized(fn)` so the AuthProvider can hook in after module load — avoids a circular import between AuthProvider and api-client. (Covered by 5 unit tests in `client.test.ts`: fires on non-auth 401, skips for `/auth/*`, skips on opt-out, runtime replacement, no-op on 2xx.)
...
- [x] **bank** — `requests`, `approvals`, `verification.$id`, `index` (dashboard) to API. `messages`, `reports` → `<ComingSoon />`. Deleted `apps/bank/src/lib/api.ts` (mock-wiring shim), `lib/mock-data.ts`, `lib/types.ts`, `lib/use-requests.ts`. New `lib/mappers.ts` defines a narrow `VerificationStatus` matching the backend's 6-value enum + `WireVerification` to absorb `@verifly/types` looseness. AppShell + AppSidebar + TopBar gated on `useAuth().user?.role === "bank"`; sidebar shows real user + sign-out; TopBar's code-search uses `apiClient.verifications.lookupByCode`. Decision dialogs in `verification.$id.tsx` call `apiClient.verifications.decide` with verifiedAmount converted from the UI's whole-currency input to the wire's minor units. 13-test mapper unit suite.
